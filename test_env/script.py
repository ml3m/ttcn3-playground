#!/usr/bin/env python3
"""
Advanced TTCN-3 Parser for log(PRINT_UC, ...) Analysis - ENHANCED VERSION
...
NEW RULE: Object Definition
- An object is defined as a string without spaces in it
- Each string parameter counts as 1 object, regardless of its content
- String parameters should be continuous writing without spaces or commas
- Examples:
  * log(PRINT_UC, "first_statement") → 1 object (valid)
  * log(PRINT_UC, "CleanupObject") → 1 object (valid)
  * log(PRINT_UC, "first statement") → 1 object (INVALID: contains space)
  * log(PRINT_UC, "obj1,obj2,obj3") → 1 object (INVALID: contains comma)
  * log(PRINT_UC, "obj1", "obj2", "obj3") → 3 objects (valid)

NEW FEATURE: Invalid String Detection
- Detects log(PRINT_UC, "obj1, obj2, obj3") format which is considered invalid
- New flag --include-invalid to include functions with invalid string formats
- Invalid functions are marked with "invalid_string" tag in outputs
...
7. UPDATED: Object counting rules:
   - Each string parameter counts as 1 object, regardless of content
   - No longer splits strings by commas to count internal objects
8. NEW: Invalid string format detection and flagging

Version: 1.5 - Added String Format Validation Rules
"""
...
class PrintUCOccurrence:
    ...
    object_count: int
    full_statement: str
    is_invalid_string: bool = False  # NEW: Flag for invalid string format
...
class FunctionInfo:
    ...
    @property
    def has_invalid_string_format(self) -> bool:
        """Check if function has log(PRINT_UC, ...) with invalid string format."""
        return any(occurrence.is_invalid_string for occurrence in self.print_uc_occurrences)
...
class TTCN3Parser:
    """Advanced TTCN-3 parser for analyzing PRINT_UC usage patterns - ENHANCED VERSION."""
    ...
    def extract_all_print_uc_occurrences(self, function_body: str, function_start_line: int) -> List[PrintUCOccurrence]:
        """
        ENHANCED: Extract ALL log(PRINT_UC, ...) occurrences from a function body.
        Now includes invalid string format detection.
        ...
        while i < len(body_lines):
            ...
            for pattern in self.print_uc_patterns:
                ...
                for match in matches:
                    ...
                    if full_statement:
                        object_count, is_invalid = self.count_print_uc_objects_enhanced(full_statement)
    ...
    def _comprehensive_print_uc_scan(self, body: str, function_start_line: int) -> List[PrintUCOccurrence]:
        ...
        for pattern in comprehensive_patterns:
            ...
            for match in matches:
                ...
                if clean_statement:
                    object_count, is_invalid = self.count_print_uc_objects_enhanced(clean_statement)
    ...
    def count_print_uc_objects_enhanced(self, print_uc_statement: str) -> Tuple[int, bool]:
        """
        ENHANCED: Count the number of objects in a log(PRINT_UC, ...) statement.
        
        NEW RULE: An object is defined as a string without spaces in it.
        This means each string parameter counts as 1 object, regardless of its content.
        
        NEW VALIDATION RULE: String parameters should be continuous writing without spaces or commas.
        Any string containing spaces or commas is considered invalid.
        
        Returns (object_count, is_invalid_string):
        - object_count: number of objects found
        - is_invalid_string: True if any string parameter contains spaces or commas
        
        Examples:
        1. log(PRINT_UC, "first_statement") - counts as 1 object (valid)
        2. log(PRINT_UC, "CleanupObject") - counts as 1 object (valid)
        3. log(PRINT_UC, "first statement") - counts as 1 object (INVALID: contains space)
        4. log(PRINT_UC, "obj1,obj2,obj3") - counts as 1 object (INVALID: contains comma)
        5. log(PRINT_UC, "obj1", "obj2", "obj3") - counts as 3 objects (valid)
        """
        ...
        if content is None or not content.strip():
            return 0, False  # log(PRINT_UC) with no additional objects
        
        content = content.strip()
        
        # Parse the parameters to the log function
        parameters = self._parse_log_parameters(content)
        
        # NEW RULE: Each parameter counts as 1 object, regardless of its content
        # This means we simply count the number of parameters
        result = len(parameters)
        
        # NEW VALIDATION: Check if any string parameter contains spaces or commas
        is_invalid_string = False
        for param in parameters:
            param = param.strip()
            if self._is_string_literal(param):
                # Extract the content inside the string
                ...
                if string_content:
                    ...
                    if ' ' in string_content or ',' in string_content:
                        print(f"DEBUG: INVALID STRING FORMAT detected - String contains spaces or commas: {param}")
        ...
        return result, is_invalid_string
    
    def count_print_uc_objects_fixed(self, print_uc_statement: str) -> int:
        """
        LEGACY: Keep the old method for backward compatibility.
        This method only returns the object count without invalid format detection.
        """
        object_count, _ = self.count_print_uc_objects_enhanced(print_uc_statement)
        return object_count
    ...
    def _count_objects_in_string(self, string_content: str) -> int:
        """
        DEPRECATED: This method is no longer used with the new object counting rules.
        
        NEW RULE: An object is defined as a string without spaces in it.
        Each string parameter counts as 1 object, regardless of its content.
        
        This method is kept for backward compatibility but always returns 1.
        """
        # NEW RULE: Each string parameter counts as 1 object, regardless of content
        # This means we no longer split by commas or count internal objects
        return 1
    ...
    def parse_functions(self, content: str, include_testcases: bool = False) -> List[FunctionInfo]:
        ...
        for function_type_str, function_name, start_line, brace_line, brace_pos in function_declarations:
            ...
            # ENHANCED: Find ALL PRINT_UC occurrences in the function body
    ...
    def parse_file(self, file_path: str, debug: bool = False, include_testcases: bool = False) -> List[FunctionInfo]:
        """ENHANCED: Parse a single TTCN-3 file with enhanced error handling and invalid string detection."""
        try:
                    ...
                    for func in functions:
                        ...
                        if func.print_uc_occurrences:
                            ...
                            for i, occ in enumerate(func.print_uc_occurrences):
                                ...
                                if occ.is_invalid_string:
                                    print_uc_info += " - INVALID STRING FORMAT"
                                print_uc_info += "]"
                        
                        if func.has_invalid_string_format:
                            invalid_info = " ⚠️ INVALID STRING FORMAT"
                        
                        print(f"      - {func.function_type.value} {func.name}{print_uc_info}{invalid_info}")
                        ...
                        if func.print_uc_occurrences:
                            ...
                            print(f"        → has_invalid_string_format: {func.has_invalid_string_format}")
    ...
    def parse_directory(self, directory_path: str, recursive: bool = False, debug: bool = False, include_testcases: bool = False) -> Dict[str, List[FunctionInfo]]:
        """ENHANCED: Parse all TTCN-3 files in a directory with comprehensive file detection."""
        results = {}
...
class ResultFormatter:
    ...
    @staticmethod
    def format_function_info(func_info: FunctionInfo, include_invalid: bool = False) -> str:
        ...
        if func_info.print_uc_occurrences:
            ...
            for occurrence in func_info.print_uc_occurrences:
                detail = f"Line {occurrence.line_number}: {occurrence.object_count} objects"
                if occurrence.is_invalid_string:
                    detail += " (INVALID STRING FORMAT)"
                print_uc_details.append(detail)
            print_uc_info = f" (PRINT_UC: {', '.join(print_uc_details)})"
        
        if include_invalid and func_info.has_invalid_string_format:
            invalid_marker = " ⚠️ [INVALID STRING FORMAT]"
        
        return f"  {type_str} {func_info.name} [Lines {func_info.start_line}-{func_info.end_line}]{print_uc_info}{invalid_marker}"
    
    @staticmethod
    def print_results(results: Dict[str, List[FunctionInfo]], include_testcases: bool = False, include_invalid: bool = False):
        """Print formatted results for both parts, optionally including invalid string format functions."""
        if not results:
        ...
        if include_invalid:
            print("(Including Invalid String Format Detection)")
        print("=" * 80)
        ...
        if part1_functions:
            ...
            for file_path, functions in part1_by_file.items():
                ...
                for func in sorted(functions, key=lambda x: x.start_line):
                    print(ResultFormatter.format_function_info(func, include_invalid))
        else:
            print("  None found.")
            ...
            for file_path, functions in part2_by_file.items():
                ...
                for func in sorted(functions, key=lambda x: x.start_line):
                    ...
                    for occurrence in func.print_uc_occurrences:
                        invalid_marker = " [INVALID STRING FORMAT]" if occurrence.is_invalid_string else ""
                        print(f"    → {occurrence.full_statement}{invalid_marker}")
        else:
            print("  None found.")
        
        # Part 3 (NEW): Functions/altsteps WITH invalid string format (if include_invalid is True)
        if include_invalid:
            invalid_functions = [f for f in all_functions if f.has_invalid_string_format]
            
            print(f"\nPART 3: {entity_type.title()} WITH INVALID STRING FORMAT ({len(invalid_functions)} found)")
            print("-" * 80)
            ...
            if invalid_functions:
                ...
                for func in invalid_functions:
                    ...
                    for file_path, file_functions in results.items():
                        if func in file_functions:
                            if file_path not in invalid_by_file:
                                invalid_by_file[file_path] = []
                            invalid_by_file[file_path].append(func)
                            break
                
                for file_path, functions in invalid_by_file.items():
                    print(f"\nFile: {os.path.basename(file_path)}")
                    for func in sorted(functions, key=lambda x: x.start_line):
                        print(ResultFormatter.format_function_info(func, include_invalid))
                        
                        # Show invalid PRINT_UC statements
                        invalid_occurrences = [occ for occ in func.print_uc_occurrences if occ.is_invalid_string]
                        for occurrence in invalid_occurrences:
                            print(f"    → INVALID: {occurrence.full_statement}")
                            print(f"      (Multiple objects in single string - should be separate parameters)")
            else:
                print("  None found.")
        
        # Summary statistics - enhanced to include invalid format info
        print(f"\n" + "=" * 80)
        ...
        functions_invalid = len([f for f in all_functions if f.has_invalid_string_format]) if include_invalid else 0
        ...
        if include_invalid:
            print(f"{entity_type.title()} with INVALID STRING FORMAT: {functions_invalid}")
        ...
        for file_path, functions in results.items():
            if functions:
                invalid_count = len([f for f in functions if f.has_invalid_string_format]) if include_invalid else 0
                invalid_info = f" ({invalid_count} with invalid format)" if include_invalid and invalid_count > 0 else ""
                print(f"  {os.path.basename(file_path)} ({len(functions)} candidates{invalid_info})")
        ...
        if include_invalid and functions_invalid > 0:
            print(f"⚠️ Invalid string format candidates found: {functions_invalid}")
        print("=" * 80)
    
    @staticmethod
    def export_results(results: Dict[str, List[FunctionInfo]], output_file: str, only_names: bool = False, include_invalid: bool = False):
        """Export results to a file in JSON format with optional invalid string format info."""
        if not results:
        ...
        # Part 1: Functions/altsteps WITHOUT PRINT_UC (VALID ONLY)
        part1_functions = [f for f in all_functions if not f.has_print_uc and not f.has_invalid_string_format]
        
        # Part 2: Functions/altsteps WITH multiple PRINT_UC or multiple objects (VALID ONLY)
        part2_functions = [f for f in all_functions if f.qualifies_for_part2 and not f.has_invalid_string_format]
        
        # Part 3: Functions/altsteps WITH invalid string format (if requested)
        invalid_functions = [f for f in all_functions if f.has_invalid_string_format] if include_invalid else []
        ...
        if only_names:
            ...
            if include_invalid:
                export_data["summary"]["functions_with_invalid_format"] = len(invalid_functions)
                export_data["invalid_format_function_names"] = [func.name for func in invalid_functions]
        else:
            ...
            if include_invalid:
                export_data["summary"]["functions_with_invalid_format"] = len(invalid_functions)
                export_data["invalid_format_functions"] = []
            ...
            for func in part1_functions:
                for file_path, file_functions in results.items():
                    if func in file_functions:
                        ...
                        if include_invalid:
                            func_data["has_invalid_string_format"] = func.has_invalid_string_format
                        part1_by_file[file_path].append(func_data)
                        break
                ...
                for file_path, file_functions in results.items():
                    if func in file_functions:
                        ...
                        for occurrence in func.print_uc_occurrences:
                            ...
                            if include_invalid:
                                occ_data["is_invalid_string"] = occurrence.is_invalid_string
                            func_data["print_uc_occurrences"].append(occ_data)
            ...
            # Add invalid format functions if requested
            if include_invalid:
                invalid_by_file = {}
                for func in invalid_functions:
                    for file_path, file_functions in results.items():
                        if func in file_functions:
                            ...
                            # Add invalid PRINT_UC details
                            for occurrence in func.print_uc_occurrences:
                                if occurrence.is_invalid_string:
                                    func_data["invalid_occurrences"].append({
                                        "line_number": occurrence.line_number,
                                        "object_count": occurrence.object_count,
                                        "statement": occurrence.full_statement,
                                        "reason": "Multiple objects in single string parameter"
                                    })
                            
                            invalid_by_file[file_path].append(func_data)
                            break
                
                export_data["invalid_format_functions"] = invalid_by_file
            ...
            for file_path, functions in results.items():
                for func in functions:
                    ...
                    if include_invalid:
                        func_detail["has_invalid_string_format"] = func.has_invalid_string_format
        ...
        try:
            ...
            if include_invalid and invalid_functions:
                print(f"  Including {len(invalid_functions)} functions with invalid string format")
        except Exception as e:
            print(f"Error exporting results to '{output_file}': {e}")
    
    @staticmethod
    def export_part1_results(results: Dict[str, List[FunctionInfo]], output_file: str, only_names: bool = False, include_invalid: bool = False):
        ...
        # Part 1: Functions/altsteps WITHOUT PRINT_UC (VALID ONLY)
        # Exclude functions with invalid string format from Part 1
        part1_functions = [f for f in all_functions if not f.has_print_uc and not f.has_invalid_string_format]
        
        if not part1_functions:
            print("No valid Part 1 functions (without PRINT_UC) found to export.")
            return
        ...
        if only_names:
            ...
            export_data = {
                "summary": {
                    ...
                    "description": "Functions and altsteps WITHOUT PRINT_UC statements (VALID ONLY)"
                },
                "function_names": []
            }
            
            for func in part1_functions:
                export_data["function_names"].append(func.name)
        else:
            # Prepare detailed export data
            export_data = {
                "summary": {
        ...
        try:
            ...
            if only_names:
                ...
                print(f"Part 1 results (no PRINT_UC, valid only) exported to '{output_file}' ({len(part1_functions)} functions)")
        except Exception as e:
            print(f"Error exporting Part 1 results to '{output_file}': {e}")
    
    @staticmethod
    def export_part2_results(results: Dict[str, List[FunctionInfo]], output_file: str, only_names: bool = False, include_invalid: bool = False):
        ...
        # Part 2: Functions/altsteps WITH multiple PRINT_UC or multiple objects (VALID ONLY)
        # Exclude functions with invalid string format from Part 2
        part2_functions = [f for f in all_functions if f.qualifies_for_part2 and not f.has_invalid_string_format]
        
        if not part2_functions:
            print("No valid Part 2 functions (with multiple PRINT_UC objects/statements) found to export.")
            return
        ...
        if only_names:
            ...
            export_data = {
                "summary": {
                    ...
                    "description": "Functions and altsteps WITH multiple objects in PRINT_UC OR multiple PRINT_UC statements (VALID ONLY)"
                },
                "function_names": []
            }
        ...
        try:
            ...
            if only_names:
                ...
                print(f"Part 2 results (with PRINT_UC rules, valid only) exported to '{output_file}' ({len(part2_functions)} functions)")
        except Exception as e:
            print(f"Error exporting Part 2 results to '{output_file}': {e}")
    
    @staticmethod
    def export_invalid_results(results: Dict[str, List[FunctionInfo]], output_file: str, only_names: bool = False):
        """Export functions/altsteps WITH invalid string format to JSON file."""
        if not results:
            print("No results to export for invalid string format.")
            return
        ...
        # Functions/altsteps WITH invalid string format
        invalid_functions = [f for f in all_functions if f.has_invalid_string_format]
        
        if not invalid_functions:
            print("No functions with invalid string format found to export.")
            return
        ...
        if only_names:
            ...
            export_data = {
                "summary": {
                    "total_invalid_functions": len(invalid_functions),
                    "description": "Functions and altsteps WITH invalid string format in PRINT_UC statements"
                },
                "function_names": [func.name for func in invalid_functions]
            }
        else:
            # Prepare detailed export data
            export_data = {
                ...
                "invalid_format_functions": {}
            }
            
            # Group invalid functions by file
            invalid_by_file = {}
            for func in invalid_functions:
                for file_path, file_functions in results.items():
                    if func in file_functions:
        ...
        try:
            with open(output_file, 'w', encoding='utf-8') as f:
                json.dump(export_data, f, indent=2, ensure_ascii=False)
            if only_names:
                print(f"Invalid format function names exported to '{output_file}' ({len(invalid_functions)} functions)")
            else:
                print(f"Invalid format results exported to '{output_file}' ({len(invalid_functions)} functions)")
        except Exception as e:
            print(f"Error exporting invalid format results to '{output_file}': {e}")
...
def main():
    ...
    arg_parser = argparse.ArgumentParser(
        description="Advanced TTCN-3 Parser for log(PRINT_UC, ...) Analysis - ENHANCED VERSION",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s /path/to/ttcn3/code                     # Parse directory (functions/altsteps only)
  %(prog)s --include-testcases /path/to/ttcn3/code # Parse directory including testcases
  %(prog)s --include-invalid /path/to/ttcn3/code   # Parse and detect invalid string formats
  ...
  %(prog)s --out-no-pfs part1.json .               # Export only Part 1 (no log(PRINT_UC, ...))
  %(prog)s --out-pfs part2.json .                  # Export only Part 2 (log(PRINT_UC, ...) rules)
  %(prog)s --out-invalid invalid.json .            # Export only invalid string format functions
  %(prog)s --include-invalid --out-invalid inv.json . # Detect and export invalid formats
  %(prog)s --out-no-pfs p1.json --out-pfs p2.json . # Export both parts separately
  %(prog)s -r -o full.json --include-invalid .     # Recursive with invalid detection
  %(prog)s single_file.ttcn                        # Parse single file
  %(prog)s --debug /path/to/ttcn3/code              # Enable detailed debug output
  %(prog)s --only-names -o names.json .             # Export only function names
  %(prog)s --only-names --include-invalid --out-invalid inv.json . # Export invalid names only
  %(prog)s --default .                              # Create all outputs in organized directory
  %(prog)s --default -r /path/to/ttcn3/code        # Default mode with recursive search

PRINT_UC Object Counting Rules:
  log(PRINT_UC, "first_statement")      → 1 object  (valid: continuous writing)
  log(PRINT_UC, "CleanupObject")        → 1 object  (valid: continuous writing)
  log(PRINT_UC, "first statement")      → 1 object  (INVALID: contains space)
  log(PRINT_UC, "obj1,obj2,obj3")       → 1 object  (INVALID: contains comma)
  log(PRINT_UC, "obj1", "obj2", "obj3") → 3 objects (valid: separate parameters)
  
  NEW RULE: An object is defined as a string without spaces in it.
  Each string parameter counts as 1 object, regardless of its content.
  NEW VALIDATION: String parameters should be continuous writing without spaces or commas.
  
Invalid String Format Detection:
  The --include-invalid flag enables detection of strings containing spaces or commas,
  which are considered invalid formats.
        """,
    )
    ...
    arg_parser.add_argument(
        '--out-invalid',
        metavar='filename.json',
        help='Export functions/altsteps WITH invalid string format to JSON file (requires --include-invalid)'
    )
    arg_parser.add_argument(
        '--include-invalid',
        action='store_true',
        help='Detect and include functions with invalid string format in log(PRINT_UC, "obj1, obj2, obj3")'
    )
    arg_parser.add_argument(
        '--debug',
        action='store_true',
    ...
    arg_parser.add_argument(
        '--default',
        action='store_true',
        help='Use default output mode: creates all possible outputs (names and detailed JSON) in an organized output directory'
    )
    ...
    if args.out_invalid and not args.include_invalid:
        print("Error: --out-invalid requires --include-invalid to be specified.")
        return 1
    
    # Handle --default flag
    if args.default:
        # Create output directory
        output_dir = "ttcn3_parser_output"
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
            print(f"Created output directory: {output_dir}")
        
        # Set default output files
        args.out_no_pfs = os.path.join(output_dir, "part1_no_print_uc.json")
        args.out_pfs = os.path.join(output_dir, "part2_with_print_uc.json")
        args.out_invalid = os.path.join(output_dir, "invalid_string_format.json")
        args.output = os.path.join(output_dir, "complete_results.json")
        
        # Also create names-only versions
        args.only_names = False  # We want both detailed and names-only
        
        # Enable invalid detection for default mode
        args.include_invalid = True
        
        print(f"Default mode enabled: All outputs will be saved to '{output_dir}/' directory")
        print(f"  - Part 1 (no PRINT_UC): {args.out_no_pfs}")
        print(f"  - Part 2 (with PRINT_UC): {args.out_pfs}")
        print(f"  - Invalid formats: {args.out_invalid}")
        print(f"  - Complete results: {args.output}")
        print(f"  - Names-only files will also be created")
    
    # Initialize parser and formatter
    ttcn_parser = TTCN3Parser()
    formatter = ResultFormatter()
    
    print("Advanced TTCN-3 Parser for PRINT_UC Analysis - ENHANCED VERSION v1.3")
    print("=" * 70)
    ...
    if args.include_invalid:
        print("Mode: Invalid string format detection enabled")
    if args.output:
    ...
    if args.out_invalid:
        print(f"Invalid format output: {args.out_invalid}")
    if args.debug:
    ...
    print("\nPRINT_UC Object Counting Rules:")
    print('  log(PRINT_UC, "first_statement")      → 1 object  (valid: continuous writing)')
    print('  log(PRINT_UC, "CleanupObject")        → 1 object  (valid: continuous writing)')
    print('  log(PRINT_UC, "first statement")      → 1 object  (INVALID: contains space)')
    print('  log(PRINT_UC, "obj1,obj2,obj3")       → 1 object  (INVALID: contains comma)')
    print('  log(PRINT_UC, "obj1", "obj2", "obj3") → 3 objects (valid: separate parameters)')
    print('  NEW RULE: An object is defined as a string without spaces in it.')
    print('  Each string parameter counts as 1 object, regardless of its content.')
    print('  NEW VALIDATION: String parameters should be continuous writing without spaces or commas.')
    if args.include_invalid:
        print("\n⚠️ Invalid string format detection is ENABLED")
        print("   Functions using strings with spaces or commas will be flagged as invalid")
    print()
    ...
    # Display results
    formatter.print_results(results, args.include_testcases, args.include_invalid)
    
    # Export results if output files specified
    export_performed = False
    if args.output:
        print()  # Add spacing before export message
        formatter.export_results(results, args.output, args.only_names, args.include_invalid)
        export_performed = True
    
    if args.out_no_pfs:
        if not export_performed:
            print()  # Add spacing before export message
        formatter.export_part1_results(results, args.out_no_pfs, args.only_names, args.include_invalid)
        export_performed = True
    
    if args.out_pfs:
        if not export_performed:
            print()  # Add spacing before export message
        formatter.export_part2_results(results, args.out_pfs, args.only_names, args.include_invalid)
        export_performed = True
    
    if args.out_invalid:
        if not export_performed:
            print()  # Add spacing before export message
        formatter.export_invalid_results(results, args.out_invalid, args.only_names)
        export_performed = True
    
    # Create names-only versions if using --default flag
    if args.default:
        print()  # Add spacing before names-only export message
        print("Creating names-only versions...")
        
        # Create names-only versions in the same output directory
        output_dir = "ttcn3_parser_output"
        
        # Names-only Part 1
        names_part1_file = os.path.join(output_dir, "part1_no_print_uc_names.json")
        formatter.export_part1_results(results, names_part1_file, only_names=True, include_invalid=args.include_invalid)
        
        # Names-only Part 2
        names_part2_file = os.path.join(output_dir, "part2_with_print_uc_names.json")
        formatter.export_part2_results(results, names_part2_file, only_names=True, include_invalid=args.include_invalid)
        
        # Names-only Invalid
        names_invalid_file = os.path.join(output_dir, "invalid_string_format_names.json")
        formatter.export_invalid_results(results, names_invalid_file, only_names=True)
        
        # Names-only Complete
        names_complete_file = os.path.join(output_dir, "complete_results_names.json")
        formatter.export_results(results, names_complete_file, only_names=True, include_invalid=args.include_invalid)
        
        print(f"✅ All outputs created successfully in '{output_dir}/' directory")
        print(f"📁 Detailed JSON files: 4 files")
        print(f"📄 Names-only JSON files: 4 files")
        print(f"📋 Total output files: 8 files")
